...
class Options extends Component{
	...
	componentWillMount() { 
		console.log(this.props.location.pathname.split('/').slice(1, 4));
    ...
    onSelect = (opt) => {
        ...
        if(opt.props.value==2){
            this.props.history.push(this.props.match.path+"/about");
        }
    }
    ...
    nextStep(){
    	if(this.state.tabIdx==0){
    		...
    		this.props.history.push(this.props.match.path+"/carConfig");
    	}
    }
	render(){
		...
		return (
			<div className="container options-page">
				...
				<AnimatedRouter timeout={300}  transitionKey={this.props.location.pathname.split('/').slice(0, 4).join('/')}>
					<Route path={routerRoot+"/help"}  component={Help}/>
	         		<Route path={routerRoot+"/about"} component={About}/>
	         		<Route path={routerRoot+"/login"} component={Login}/>
	         		<Route path={routerRoot+"/carConfig"}  component={CarConfig}/>
				</AnimatedRouter>
			</div>
		)
	}
